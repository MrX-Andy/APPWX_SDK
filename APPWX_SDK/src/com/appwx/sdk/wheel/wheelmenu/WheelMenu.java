package com.appwx.sdk.wheel.wheelmenu;
...
public class WheelMenu {
	private Bitmap imageOriginal, imageScaled; // variables for original and
												// resized image
	private Context context; // variable to store the context of the calling
								// activity
	private Matrix matrix; // Matrix used to perform rotations
	private ImageView wheel; // image view which holds the wheel
	private int wheelHeight, wheelWidth; // height and width of the image view
											// containing the wheel
	private int top; // the current top of the wheel
	private double total_rotation; // variable that counts the total rotation
									// during a given rotation of the wheel by
									// the user (from ACTION_DOWN to ACTION_UP)
	private int divcount; // variable that counts the no of divisons in the
							// wheel
	...
	private boolean snapFlag = true; // variable that determines whether to snap
										// the wheel to the center of a div or
										// not
	...
	/*
	 * constructor form: WheelMenu(Context, no_of_divisions_in_the_menu,
	 * drawable_to_use_as_wheel_img, imageview_for_the_wheel,
	 * flag_to_set_snap_to_center_option)
	 */
	public WheelMenu(Context context_par, int divcount_par, int drawable_id,
			ImageView wheel_par, boolean snapFlag_par) {
		...
		if (imageOriginal == null) {
			imageOriginal = BitmapFactory.decodeResource(
					context.getResources(), drawable_id);
		}
		...
		/*
		 * listener to find out when layout is initialized. We need this to get
		 * the dimensions of the image view. Once we get those, We can scale the
		 * image to make sure it's proper, Initialize the matrix and align it
		 * with the image view's center.
		 */
		wheel.getViewTreeObserver().addOnGlobalLayoutListener(
				new ViewTreeObserver.OnGlobalLayoutListener() {
					@Override
					public void onGlobalLayout() {
						// method called multiple times but we can initialize
						// just once
	...
	private class WheelTouchListener implements View.OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_UP:
				...
				// of degrees inside the current top
